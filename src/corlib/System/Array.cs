...
#nullable disable
        ...
        public static Array<T> CreateInstance<T>(int length) where T : unmanaged
        {
            // Aquí deberías usar RhpNewArray con el EEType de T
            EETypePtr eeType = EETypePtr.EETypePtrOf<T>();
            return (Array<T>)StartupCodeHelpers.RhpNewArray(eeType._value, length);
        }
...
#nullable disable
        ...
        public static int IndexOf<T>(T[] array, T value, int startIndex = 0)
        {
            if (array == null)
            {
                ThrowHelpers.ThrowArgumentNullException("array");
        ...
        // Método auxiliar para comparación de igualdad
        private static bool EqualityComparer<T>(T a, T b)
        {
            // Para tipos de valor, usar comparación directa
            if (typeof(T).IsValueType)
            {
                return a.Equals(b);
            }

            // Para tipos de referencia, manejar casos especiales
            if (a == null && b == null)
                return true;
